...
export const readingAPI = {
    ...
    interpretReading: async (
        ...
        const SPREAD_TYPE_MAP: Record<string, string> = {
            'three-card': 'THREE_CARD',
            'celtic-cross': 'CELTIC_CROSS',
        };

        const mappedSpreadType = SPREAD_TYPE_MAP[spreadType];
        if (!mappedSpreadType) {
            throw new Error(`Unsupported spread type: '${spreadType}'. Expected one of: ${Object.keys(SPREAD_TYPE_MAP).join(', ')}`);
        }
