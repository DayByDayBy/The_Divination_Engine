...
describe('InterpretRequestSchema', () => {
    ...
    it('requires spreadType enum: THREE_CARD, CELTIC_CROSS', () => {
        const base = {
            readingId: 1,
            userInput: 'test',
        ...
        for (const st of ['THREE_CARD', 'CELTIC_CROSS']) {
    ...
    it('rejects previously-supported but now-removed spread types', () => {
        const base = {
            readingId: 1,
            userInput: 'test',
            cards: [{ name: 'The Fool', reversed: false, meaningUp: 'a', meaningRev: 'b', position: 0 }],
        ...
        expect(InterpretRequestSchema.safeParse({ ...base, spreadType: 'ONE_CARD' }).success).toBe(false);
        expect(InterpretRequestSchema.safeParse({ ...base, spreadType: 'CUSTOM' }).success).toBe(false);
    });
